...
pub fn quote_opt_string<T: ToTokens>(src: &Option<T>) -> TokenStream {
    if let Some(string) = src.as_ref() {
        quote! { Some(conf::const_str(#string)) }
    } else {
        quote! { None }
    }
}

/// Helper for turning values Option<LitChar> into Option<&'static str>
pub fn quote_opt_char(src: &Option<LitChar>) -> TokenStream {
    quote_opt(&src.as_ref().map(|c| c.value().to_string()))
}

/// Helper for turning values Option<LitChar> into Option<conf::CowStr>
pub fn quote_opt_char_string(src: &Option<LitChar>) -> TokenStream {
    quote_opt_string(&src.as_ref().map(|c| c.value().to_string()))
}
